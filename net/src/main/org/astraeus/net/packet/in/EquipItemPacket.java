			...
			if (equipDef != null) {
				...
				if (equipDef.isTwoHanded() && player.getEquipment().getItems()[Equipment.SHIELD] != null) {
					if (player.getInventory().getFreeSlots() < 1) {
						player.send(new ServerMessagePacket(
								"You don't have the required inventory space to equip this item."));
						return;
					}
				}
				...
				if (itemDef != null) {
					...
					if (player.getEquipment().getItems()[equipDef.getType().getSlot()] != null
							&& itemDef.isStackable()) {
						if (!player.getEquipment().contains(id)) {
							player.getInventory().set(slot, new Item(
									player.getEquipment().getItems()[equipDef.getType().getSlot()].getId(),
					...
					if (ItemDefinition.getDefinitions()[id].isStackable()) {
						if (player.getEquipment().contains(id)) {
							player.getEquipment().getItems()[equipDef.getType().getSlot()].setAmount(
									player.getEquipment().getItems()[equipDef.getType().getSlot()].getAmount()
											+ stackableAmount);
						} else {
						...
						if (equipDef.isTwoHanded() && player.getEquipment().getItems()[Equipment.SHIELD] != null) {
							player.getInventory()
									.add(new Item(player.getEquipment().getItems()[Equipment.SHIELD].getId(), 1));
							...
							player.getInventory()
									.add(new Item(player.getEquipment().getItems()[Equipment.WEAPON].getId(), 1));
